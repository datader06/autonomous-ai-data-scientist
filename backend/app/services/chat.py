...
class DatasetChatService:
    ...
    def _dataset_documents(self, dataframe: pd.DataFrame) -> list[Document]:
        ...
        documents.append(
            Document(
                page_content="\n".join(profile_lines),
                metadata={"source": "dataset_profile"},
            )
        )

        for row_index, row in dataframe.head(250).iterrows():
            row_text = "\n".join(
                [f"{column}: {row[column]}" for column in dataframe.columns]
            )
    ...
    def answer(self, dataset_id: str, question: str) -> dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You answer questions about an uploaded tabular dataset. "
        ...
        sources = [
            doc.metadata.get("source", "dataset")
